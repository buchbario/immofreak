...
export function DunningLetter({ onBack }: Props) {
  ...
  const { bankInfoBlock, settings, isComplete } = useLandlordSettings();
        ...
        <div className="flex-1 flex justify-center overflow-auto pb-8">
          <div className="origin-top" style={{ transform: 'scale(var(--a4-scale, 0.75))' }}>
            {ready ? (
              ...
              >
                {/* Stage-Badge bleibt als visueller Marker */}
                <div style={{
                  display: 'inline-flex', alignItems: 'center', gap: '8px',
                  padding: '5px 10px', borderRadius: '5px',
                  background: config.color, color: '#ffffff',
                  fontSize: '9.5px', fontWeight: 700, letterSpacing: '0.6px',
                  textTransform: 'uppercase', marginBottom: '14px',
                }}>
                  <span style={{
                    display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                    width: '16px', height: '16px', borderRadius: '50%',
                    background: 'rgba(255,255,255,0.25)', fontSize: '10px', fontWeight: 800,
                  }}>{stage}</span>
                  Stufe {stage} von 3
                </div>
                ...
                <div style={{ marginBottom: '16px', borderRadius: '6px', border: '1px solid #e5e7eb', overflow: 'hidden' }}>
                  ...
                  {items.map((it) => (
                    <div key={it.id} style={{ display: 'grid', gridTemplateColumns: '1fr 110px 110px', padding: '7px 12px', borderTop: '1px solid #f3f4f6', fontSize: '10.5px' }}>
                      <span>{it.description}</span>
                      <span style={{ fontSize: '10px', color: '#6b7280', fontVariantNumeric: 'tabular-nums' }}>{formatDate(it.dueDate)}</span>
                      <span style={{ fontWeight: 500, fontVariantNumeric: 'tabular-nums', textAlign: 'right' }}>{fmt(it.amount)} €</span>
                    </div>
                  ))}
                  {fee > 0 && (
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 110px 110px', padding: '7px 12px', borderTop: '1px solid #f3f4f6', fontSize: '10.5px', color: '#6b7280' }}>
                      <span>Mahngebühr</span>
                      <span />
                      <span style={{ fontVariantNumeric: 'tabular-nums', textAlign: 'right' }}>{fmt(fee)} €</span>
                    </div>
                  )}
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 110px 110px', padding: '10px 12px', borderTop: '2px solid #e5e7eb', fontSize: '11.5px', fontWeight: 700, background: '#fafbfc' }}>
                    ...
                    <span />
                    <span style={{ fontVariantNumeric: 'tabular-nums', textAlign: 'right', color: stage === 3 ? '#dc2626' : '#0f172a' }}>{fmt(grandTotal)} €</span>
                  </div>
                </div>
                ...
                {stage >= 2 && (
                  <div style={{ marginBottom: '14px', padding: '10px 12px', border: '1px solid #e5e7eb', borderRadius: '6px', fontSize: '9.5px', lineHeight: '1.5', color: '#4b5563', background: '#fafbfc' }}>
                    <div style={{ fontWeight: 700, marginBottom: '4px', color: '#0f172a', fontSize: '10px' }}>Rechtliche Hinweise</div>
