        ...
        <div className="flex-1 flex justify-center overflow-auto pb-8">
          <div className="origin-top" style={{ transform: 'scale(var(--a4-scale, 0.75))' }}>
            ...
            >
              {ready ? (
                <>
                  <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '4px', background: 'linear-gradient(90deg, #4F6BFF, #10b981)' }} />
                  ...
                  <div style={{ fontSize: '10px', color: '#6b7280', marginBottom: '18px' }}>
                    Rechtsgrundlage: § 560 Abs. 4 BGB &nbsp;|&nbsp; Abrechnungsjahr {billingYear}
                  </div>
                  ...
                  <p style={{ fontSize: '11px', lineHeight: '1.7', marginBottom: '18px' }}>
                    nach der Betriebs- und Heizkostenabrechnung für das Kalenderjahr <strong>{billingYear}</strong> passen wir die monatliche Vorauszahlung gemäß § 560 Abs. 4 BGB auf eine angemessene Höhe an. Die Anpassung ist erforderlich, da die tatsächlichen Kosten von den bisherigen Vorauszahlungen abweichen.
                  </p>
                  ...
                  <div style={{ marginBottom: '20px', padding: '14px 16px', background: '#f8f9fb', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10.5px' }}>
                      <tbody>
                        <tr>
                          <td style={{ padding: '4px 0' }}>Tatsächliche Kosten {billingYear}</td>
                          <td style={{ padding: '4px 0', textAlign: 'right', fontWeight: 600 }}>{fmt(actualCosts)} € / Jahr</td>
                        </tr>
                        <tr>
                          <td style={{ padding: '4px 0' }}>Monatlicher Schnitt (÷ 12)</td>
                          <td style={{ padding: '4px 0', textAlign: 'right', fontWeight: 600 }}>{fmt(monthlyActual)} € / Monat</td>
                        </tr>
                        {bufferPercent > 0 && (
                          <tr>
                            <td style={{ padding: '4px 0' }}>Sicherheitszuschlag ({bufferPercent} %)</td>
                            <td style={{ padding: '4px 0', textAlign: 'right', fontWeight: 600 }}>+ {fmt(monthlyActual * bufferPercent / 100)} €</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '20px' }}>
                    <div style={{ padding: '14px 16px', background: '#f3f4f6', borderRadius: '8px' }}>
                      <div style={{ fontSize: '9px', color: '#6b7280', textTransform: 'uppercase', fontWeight: 600, marginBottom: '6px' }}>Bisher</div>
                      <div style={{ fontSize: '18px', fontWeight: 700 }}>{fmt(currentAdvance)} €</div>
                      <div style={{ fontSize: '9.5px', color: '#6b7280' }}>monatlich</div>
                    </div>
                    <div style={{ padding: '14px 16px', background: diff >= 0 ? '#fef2f2' : '#ecfdf5', borderRadius: '8px', border: `1px solid ${diff >= 0 ? '#ef4444' : '#10b981'}` }}>
                      <div style={{ fontSize: '9px', color: diff >= 0 ? '#b91c1c' : '#065f46', textTransform: 'uppercase', fontWeight: 600, marginBottom: '6px' }}>Neu ab {formatDate(effectiveFrom)}</div>
                  ...
                  <div style={{ marginBottom: '20px', padding: '10px 12px', borderLeft: '3px solid #4F6BFF', background: '#f0f4ff', fontSize: '9.5px', color: '#1e40af' }}>
                    ...
                    Gemäß § 560 Abs. 4 BGB kann jede Vertragspartei nach einer Abrechnung die Vorauszahlungen auf eine angemessene Höhe anpassen. Die Anpassung muss sich an den tatsächlichen Kosten der letzten Abrechnungsperiode orientieren (BGH VIII ZR 336/09). Ein pauschaler Sicherheitszuschlag ist nur in engen Grenzen zulässig.
                  </div>
                  ...
                  <div style={{ position: 'absolute', bottom: '30px', left: '60px', right: '60px', fontSize: '8.5px', color: '#9ca3af', display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <span>§ 560 BGB — Veränderungen der Betriebskosten</span>
                  </div>
                </>
              ) : (
